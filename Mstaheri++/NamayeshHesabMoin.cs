...
{
    public partial class NamayeshHesabMoin : Form
    {
        CarpetCompanyBLL.classHesabMion Mion;
        public NamayeshHesabMoin()
        {
            InitializeComponent();
            Mion = new CarpetCompanyBLL.classHesabMion();
        }

        private async void NamayeshHesabMoin_Load(object sender, EventArgs e)
        {
           await Select();
        }
        ...
        private async void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == dataGridView1.Columns[Deletee.Name].Index)
                {
                    ...
                    {
                        string CodeSenad = dataGridView1.CurrentRow.Cells["CodeMoin"].Value.ToString();
                        await Task.Run(() =>
                        {
                            var Check = Mion.Delete(CodeSenad);
                            if (Check.Success == true)
                            {
                                MessageBox.Show(Check.Message);
                            }
