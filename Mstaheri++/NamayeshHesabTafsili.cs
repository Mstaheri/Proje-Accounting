...
{
    public partial class NamayeshHesabTafsili : Form
    {
        CarpetCompanyBLL.classHesabTafsili Tafsili;
        public NamayeshHesabTafsili()
        {
            InitializeComponent();
            Tafsili = new CarpetCompanyBLL.classHesabTafsili();
        }

        private async void NamayeshHesabTafsili_Load(object sender, EventArgs e)
        {
            await Select();
        }
        ...
        private async void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                if (e.ColumnIndex == dataGridView1.Columns[Deletee.Name].Index)
                {
                    ...
                    {
                        string CodeSenad = dataGridView1.CurrentRow.Cells["CodeTafsili"].Value.ToString();
                        await Task.Run(() =>
                        {
                            var Check = Tafsili.Delete(CodeSenad);
                            if (Check.Success == true)
                            {
                                MessageBox.Show(Check.Message);
                            }
